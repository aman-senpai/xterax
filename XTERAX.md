...
- **acp/** — Agent Client Protocol (ACP) **client**. Hosts external coding agents as first-class AI-panel backends (Claude Code / Codex / Gemini / OpenCode / custom via Settings → ACP). Rust owns process lifecycle + JSON-RPC over stdio (`src-tauri/src/modules/acp.rs`): `acp_connect`, `acp_session_new`, `acp_prompt`, `acp_cancel`, `acp_respond_permission`, `acp_set_mode`. Client capabilities: `fs/read_text_file` / `fs/write_text_file` (absolute paths, secret-path deny-list, workspace root gate on write), `terminal/*` (non-interactive subprocess with ring-buffer logs), `session/request_permission` (bridged to UI). Events: `xterax:acp-update`, `xterax:acp-permission`, `xterax:acp-status`. Frontend: `modules/acp` (types, client, presets, pure `mapUpdates`, zustand store); chat sessions carry `backend: "local" | "acp"` + `acpAgentId`; composer branches to ACP when the active session is ACP-backed; `AcpChatView` renders transcripts + permission cards. MCP server configs are passed into `session/new` for agent-side tools. PTY-scraped Claude Code path remains as fallback until ACP presets are proven.
- **ai/** — see below.
...
- **Agent** (`lib/agent.ts`): main local agent loop with `stopWhen: stepCountIs(MAX_AGENT_STEPS)` and the system prompt from `config.ts` / prompts. Bare messages always use the unified Xterax agent under the active **mode**.
- **Modes** (`lib/modes.ts`, `store/modesStore.ts`): session posture (Default / Plan / Review / custom) selected from the composer dropdown. Modes overlay instructions, optional tool allowlists, plan-mode, and permission overrides. Not the same as agents.
- **Agents** (`lib/agents.ts`): invokable specialists with `handle`, instructions, tool/shell allowlists, optional **workflow** (ordered handle list), and per-agent **modelId** / **thinkingLevel**. Mention with `@handle` (or `[agent:handle]` chips) in the composer.
- **Pipeline DSL** (`lib/pipelineDsl.ts`, `lib/pipelineCompile.ts`, `lib/pipelineOutcome.ts`, `agents/runPipeline.ts`): user-authored sequential delegation. Chains (`@a -> @b`), loops (`loop:` / `loop N:` … `break if pass|fail|always|never`). Loop break is **structured only** via `PIPELINE_OUTCOME: pass|fail|continue` (alias `PIPELINE_BREAK:`); no NL heuristics. Body step errors inside a loop become fail outcomes and do not abort the outer chain; user abort always stops everything. UI shows nested loop structure + live run path in SubagentCards.
- **Sub-agents** (`agents/runSubagent.ts`): parallel specialist runs invoked by the main agent via `run_subagent` tool (or sequentially by the @-pipeline).
